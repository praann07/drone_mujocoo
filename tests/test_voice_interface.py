...
def test_latest_parquet_uses_mtime_not_filename_sort(tmp_path, monkeypatch):
    ...
    voice_sessions/ holds three filename prefixes (trials_, chained_,
    preemption_). Alphabetically 't' > 'c' > 'p', so `sorted(glob(...))[-1]`
    always returns a trials-prefixed file if one exists, REGARDLESS of
    which file was actually written most recently - silently showing an
    old headless test run instead of a live demo session's fresh log.
    Must pick by real file modification time."""
    import os
    import plot_2d_telemetry as p2d

    monkeypatch.setattr(p2d, "SESSIONS_DIR", tmp_path)
    older_alphabetically_last = tmp_path / "trials_20260101T000000Z.parquet"
    newer_alphabetically_first = tmp_path / "chained_20260101T000000Z.parquet"
