...
from run_stage_b import load_processed, load_split, stack  # noqa: E402
...
K_M = 0.02
SELECTED_THRESHOLD = 0.2  # chosen by the parsimony rule in run_stage_b.py
...
def model():
    ...
    if not frames:
        pytest.skip("Stage A data not generated - run run_stage_a.py first")
    X, U, D = stack(frames)
